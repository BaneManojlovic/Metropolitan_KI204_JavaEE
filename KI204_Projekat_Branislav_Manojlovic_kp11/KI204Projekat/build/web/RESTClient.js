...
// Model for Zaposleni entity
    models.Zaposleni = Backbone.Model.extend({
        urlRoot: "http://localhost:19573/KI204Projekat/webresources/com.manojlovic.restprojekat.zaposleni/",
        idAttribute: 'zaposleniBr',
        defaults: {
            ime: "",
            prezime: "",
            pol: ""
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('zaposleniBr');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:19573/KI204Projekat/webresources/com.manojlovic.restprojekat.zaposleni/';
            }
    ...
    // Collection class for Zaposleni entities
    models.ZaposleniCollection = Backbone.Collection.extend({
        model: models.Zaposleni,
        url: "http://localhost:19573/KI204Projekat/webresources/com.manojlovic.restprojekat.zaposleni/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ZaposleniCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-zaposleni-list-itemis template identifier for item
                        templateName: '#tpl-zaposleni-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.zaposleni = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.zaposleni,
                    // tpl-zaposleni-details is template identifier for chosen model element
                    templateName: '#tpl-zaposleni-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Zaposleni();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-zaposleni-details is a template identifier for chosen model element
                templateName: '#tpl-zaposleni-details',
